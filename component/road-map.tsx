...
const steps: Step[] = [
  ...
  {
    number: 3,
    title: "NABL Accredited Labs Processing",
    description:
      "Your samples are transported in advanced temperature-controlled kits to our NABL-accredited labs for rapid, high-precision processing.",
...
function Roadmap() {
  ...
  useEffect(() => {
    const updateHeights = () => {
      const heights = stepRefs.current
        .filter(ref => ref)
        .map(ref => {
          const card = ref?.querySelector('.step-card');
    ...
    <div className="bg-gradient-to-br from-gray-50 to-gray-100 py-8 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-center max-sm:mb-5 mb-8">
          ...
          <p className="text-base sm:text-lg text-gray-600 max-w-2xl mx-auto max-sm:mt-1 mt-4">
            Your health journey, simplified in four seamless steps
          </p>
        </div>
        ...
        <div className="relative">
          {/* Wavy Connecting Line - Desktop Only */}
          <div className="hidden lg:block absolute top-20 left-0 right-0 h-20">
            <svg
              className="w-full h-full"
              viewBox="0 0 1200 100"
              preserveAspectRatio="none"
              ...
              >
                <div className="flex flex-col items-center text-center h-full">
                  ...
                  <div className="relative mb-6 flex-shrink-0">
                    <div className="absolute inset-0 rounded-full blur-lg opacity-30"></div>
                    <div className="relative w-20 h-20 sm:w-24 sm:h-24 bg-[#135c8e] rounded-full flex items-center justify-center text-white shadow-xl">
                      {step.icon}
                    </div>
                  </div>

                  {/* Content Card with Equal Height */}
                  <div 
                    className="step-card bg-white rounded-2xl shadow-lg p-5 sm:p-6 w-full hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1 flex flex-col flex-grow"
                    style={maxHeight > 0 ? { minHeight: `${maxHeight}px` } : {}}
                  >
                    <h3 className="text-lg sm:text-xl font-bold text-gray-900 mb-3 line-clamp-2 h-14 sm:h-16 flex items-center justify-center">
                      {step.title}
                    </h3>
                    <p className="text-gray-600 text-sm sm:text-base leading-relaxed flex-grow flex items-center">
                      {step.description}
                    </p>
                  </div>
                </div>
          ...
          {/* Mobile Connecting Line - Horizontal for 2 columns */}
          <div className="sm:hidden absolute top-32 left-0 right-0 h-1 mx-8">
            <div className="w-full h-full bg-gradient-to-r from-transparent via-[#135c8e] to-transparent opacity-50"></div>
          </div>

          {/* Tablet Connecting Line - Vertical for 2 columns */}
          <div className="hidden sm:block lg:hidden absolute top-0 bottom-0 left-1/2 transform -translate-x-1/2 w-1">
            <div className="w-full h-full bg-gradient-to-b from-transparent via-[#135c8e] to-transparent opacity-50"></div>
          </div>
        </div>

        {/* CTA Button */}
        <div className="mt-12 max-sm:mt-8 text-center">
          <div className="relative inline-block">
            <div className="absolute bg-gradient-to-r from-[#d9534f] to-[#e74c3c] rounded-full blur-md opacity-20"></div>

            <a
              href="#contact"
              className="flex relative bg-gradient-to-r from-[#d9534f] to-[#e74c3c] text-white px-6 py-3 sm:px-8 sm:py-4 rounded-full text-base sm:text-lg font-bold shadow-lg hover:shadow-xl transform hover:-translate-y-0.5 transition-all duration-300 group"
            >
              <div className="flex items-center space-x-2">
                <span>Get Started Today</span>
